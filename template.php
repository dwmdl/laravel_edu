...
$pathToImageDirSign = '/upload/sale/bizval/309/dgisukmcicblr3yr1hau3i1qqr7ss078/Podpis-ST-Market.png';
...
<div style="margin: 0pt; padding: <?=join('pt ', $margin); ?>pt; width: <?=$width; ?>pt; background: <?=$background; ?>">
    ...
    if ($params['BASKET_ITEMS'])
    {
        ...
        {
            if (str_contains($basketItem["NAME"], "Занесение на внутренний счет")) {
                $basketItem["NAME"] = "Пополнение лицевого счета";
                $basketItem["VAT_RATE"] = 0.20;
                $basketItemPrice = $basketItem["PRICE"] * 1.2;
            }
            ...
            foreach ($arCols as $columnId => $caption)
            {
                ...
                switch ($columnId)
                {
                    ...
                    case 'PRICE':
                        ...
                        break;
                    case 'VAT_RATE':
                        if ($basketItem['NAME'] == 'Пополнение лицевого счета') {
                            $data = '20%';
                        } else {
                            $data = roundEx($basketItem['VAT_RATE'] * 100, SALE_VALUE_PRESICION)."%";
                        }
                        break;
                    case 'SUM':
            ...
            $sum += doubleval($basketItemPrice * $basketItem['QUANTITY']);
                                ...
                                { ?>
                                    <td align="right"
                                        ...
                                        <?if ($columnId == 'SUM' || $columnId == 'PRICE'):?>
                                            <?php if ($cells[$n]['product'] == 'Пополнение лицевого счета'){
                                                $cells[$n]['vat'] = 20;
                                            }
                                            ?>
                ...
                <? if ($params["SELLER_COMPANY_DIRECTOR_POSITION"]) { ?>
                    <tr>
                        ...
                        <td style="width: 160pt; border: 1pt solid #000000; border-width: 0pt 0pt 1pt 0pt; text-align: center; ">
                            <? if (!$blank) { ?>
                                <img src="<?=CFile::GetPath($params["SELLER_COMPANY_DIR_SIGN"]); ?>" width="200" height="50" />
                            <? } ?>
                        </td>
                        <td>
